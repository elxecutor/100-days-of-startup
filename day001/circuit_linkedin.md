...
Built a low-noise sensor preamp on a 3.3V EdgeNode using an LM324: AC-coupled input, mid-rail bias, and a modest gain for mV-level signals. Simulated in LTspice to verify gain and mains rejection.

How to reproduce:
- Save `ecg_frontend.cir` into LTspice and set `VCC = 3.3V`.
- Replace the ideal amplifier with a realistic LM324 model to validate output swing.

Suggested metrics to report:
- Gain at target band
- Attenuation at mains (50/60 Hz)
- Output swing under expected load

Short caption for LinkedIn (1 sentence):
Low-cost sensor preamp for 3.3V EdgeNode — LM324-based, simulated in LTspice. DM for the netlist or PCB files.

Day 001 variant: LM324 sensor preamp on 3.3V EdgeNode — DM for a compact demo build.
